...
class Payload(Openc2Type):
	...
	def getContent(self):
		try:
			for k,v in self.body['openc2lib'].items():
				return v
		except:
		  	return None
	...
	def fromdict(cls, dic, e):
		...
		for k,v in payload.body['openc2lib'].items():
			content_type = k
			content = v

		payload.body['openc2lib'][payload.getContentType()] = e.decode(Payload.getContentClass(content_type), payload.getContent())
...
class HTTPTransfer(Transfer):
	...
	def send(self, msg, encoder):
		...
		self.payload.body['openc2lib'][content] = msg.content
		...
		# msg.content-type (usually: application/openc2lib)
		...
		body = '{"headers": {"request_id": "", "created": "1712785149126", "from": "testconsumer", "to": "ciccio"}, "body": {"openc2lib": {"response": {"status": 200, "status_text": "OK", "results": {"versions": "1.0", "profiles": ["slpf"], "pairs": {"scan": ["ipv4_net"], "query": ["ipv4_net", "ipv4_connection"]}, "rate_limit": "3"}}}}}'
	...
	# Replace the following debug code!!!!
		body = '{"headers": {"request_id": "", "created": 1709914239523, "from": "ge.imati.cnr.ir", "to": ["tnt-lab.unige.it"]}, "body": {"openc2lib": {"request": {"action": "scan", "target": {"ipv4_net": "130.251.17.0/24"}}}}}'
#		body = 
