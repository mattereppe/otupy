...
otupy (/'əʊtu:paɪ/) is an open-source implementation of the OpenC2 language written in Python. It is explicitly designed with flexibility and extensibility in mind, meaning that additional profiles and actuators can be added without impacting the core library itself. For this reason, otupy is particullary suited for:
...
As a preliminary step, the necessary modules must be imported. Note that the otupy only includes core grammar and syntax elements, and all the necessary extensions (including encoders, trasfer protocols, profiles, and actuators) must be imported separetely. We will use json encoding and HTTP for our protocol stack, and an iptables actuator for stateless packet filtering:
```
import otupy as oc2
...
The server code can indeed be improved by loading the configuration from file and setting up [Logging for otupy](docs/logging.md). 
...
Advanced usage of the otupy requires knowledge of its data structures and functions. Data structures are very straightforward to learn, because they strictly follow the definition and requirements in the common Language Specification and Profile extensions. See the [Developer guide](docs/developerguide.md) for a comprehensive description of the library structure.

[comment]: <> User documentation of the otupy is available [here](docs/code/index.html).
User documentation of the otupy code can be generated by running the following command in the root tree:
```
