...
import utils
from openc2lib import Process, File
...
@pytest.mark.parametrize("pid", [random.randint(0, 65535) for i in range (0,1)])
...
def test_random_parameters(pid, name, cwd, executable, parent, command_line):
	p = Process({'name': name, 'pid': pid, 'cwd': cwd, 'executable': executable, 'parent': parent, 'command_line': command_line})
...
@pytest.mark.parametrize("args" , utils.random_params({'name': "process.exe", 'pid': 45, 'cwd': "/var/run/process", 'executable': {'name': "/usr/local/bin/executable.exe"}, 'parent': {'name':"parent", 'pid':0, 'cwd':'/var/run', 'command_line':'/usr/bin/run'} , 'command_line': 'usr/local/bin/executable.exe -a param1 -b param2'} ) )
