...
from helpers import load_yaml, load_files, send_raw_command
...
command_path_good = "../../openc2-yaml-samples/tests/commands/good"
command_path_bad = "../../openc2-yaml-samples/tests/commands/bad"
...
@pytest.mark.parametrize("cmd", load_yaml(command_path_good) )
...
def test_sending(cmd, create_yaml_producer, caplog):
...
# Filter the log to get what I need
	...
	with caplog.at_level(logging.INFO):
		resp = create_yaml_producer.sendcmd(c)
...
#		print("HTTP body: ", json.dumps(http_body))
		response = send_raw_command(http_url, http_headers, yaml.dump(http_body))
