...
from otupy.types.targets import IPv4Net, IPv6Net, MACAddr
...
def test_good_interface(name, description, if_id, ipv4_addr, ipv6_addr, mac_addr, iface_type, active):
    # Transforming inputs into the corresponding types
    transformed_ipv4_addr = ArrayOf(IPv4Net)([IPv4Net(ipv4) for ipv4 in ipv4_addr])
    transformed_ipv6_addr = ArrayOf(IPv6Net)([IPv6Net(ipv6) for ipv6 in ipv6_addr])
    ...
    obj = Interface(
        ...
        if_id=if_id,
        ipv4_addr=transformed_ipv4_addr,
        ipv6_addr=transformed_ipv6_addr,
...
@parametrize_from_file("parameters/test_interface.yml")
def test_bad_interface(bad_name, bad_description, bad_if_id, bad_ipv4_addr, bad_ipv6_addr, bad_mac_addr, bad_iface_type, bad_active):
    ...
    with pytest.raises(Exception):
        ...
        transformed_bad_ipv4_addr = ArrayOf(IPv4Net)([IPv4Net(ipv4) for ipv4 in bad_ipv4_addr]) if bad_ipv4_addr else None
        transformed_bad_ipv6_addr = ArrayOf(IPv6Net)([IPv6Net(ipv6) for ipv6 in bad_ipv6_addr]) if bad_ipv6_addr else None
        ...
        Interface(
            ...
            if_id=bad_if_id,
            ipv4_addr=transformed_bad_ipv4_addr,
            ipv6_addr=transformed_bad_ipv6_addr,
