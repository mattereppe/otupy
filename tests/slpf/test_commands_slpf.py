...
#	Deny
	if asset_id == 'iptables':
		generate_allow_deny_target_commands(asset_id, Actions.deny, src_addr_ipv4, src_addr_ipv6, dst_addr_ipv4, dst_addr_ipv6, src_port, dst_port, good_deny_commands, bad_deny_commands)
		generate_allow_deny_argument_commands(asset_id, Actions.deny, src_addr_ipv4, dst_addr_ipv4, L4Protocol.tcp, src_port, dst_port, good_deny_commands, bad_deny_commands)
#	Delete
...
#	types = [IPv4Connection]
	for t in types:
		...
		good_list.append(JSONEncoder.todict(Command(action, t(protocol=L4Protocol.udp, src_port=src_port), arg, actuator=pf)))
		good_list.append(JSONEncoder.todict(Command(action, t(protocol=L4Protocol.sctp, src_port=src_port), arg, actuator=pf)))
		good_list.append(JSONEncoder.todict(Command(action, t(protocol=L4Protocol.udp, dst_port=dst_port), arg, actuator=pf)))
		good_list.append(JSONEncoder.todict(Command(action, t(protocol=L4Protocol.sctp, dst_port=dst_port), arg, actuator=pf)))
		good_list.append(JSONEncoder.todict(Command(action, t(protocol=L4Protocol.udp, src_port=src_port, dst_port=dst_port), arg, actuator=pf)))
		good_list.append(JSONEncoder.todict(Command(action, t(protocol=L4Protocol.sctp, src_port=src_port, dst_port=dst_port), arg, actuator=pf)))

		bad_list.append(JSONEncoder.todict(Command(action, t(protocol=L4Protocol.icmp, src_port=src_port), arg, actuator=pf)))
		bad_list.append(JSONEncoder.todict(Command(action, t(protocol=L4Protocol.icmp, dst_port=dst_port), arg, actuator=pf)))
		bad_list.append(JSONEncoder.todict(Command(action, t(protocol=L4Protocol.icmp, src_port=src_port, dst_port=dst_port), arg, actuator=pf)))


def generate_allow_deny_argument_commands(asset_id, action, src_addr, dst_addr, protocol, src_port, dst_port, good_list, bad_list):
	...
	target = IPv4Connection(src_addr=src_addr, dst_addr=dst_addr, protocol=protocol, src_port=src_port, dst_port=dst_port)
	pf = slpf.Specifiers({'asset_id': asset_id})
	rule_number = 0 
	for n in range(1, len(arguments) + 1):
		for combo in itertools.combinations(arguments, n):
...
def generate_update_target_commands(asset_id, file_name_v4, file_path_v4, file_hash_md5_v4, file_hash_sha1_v4, file_hash_sha256_v4, file_name_v6, file_path_v6, file_hash_md5_v6, file_hash_sha1_v6, file_hash_sha256_v6):
	...
	for n in range(1, len(arguments) + 1):
		for combo in itertools.combinations(arguments, n):
			...
			if keys == {'name', 'hashes'}:
				...
				wrong_md5_hash = file_hash_md5_v4[:-1] + "e" if file_hash_md5_v4.endswith("f") else file_hash_md5_v4[:-1] + "f"
				wrong_sha1_hash = file_hash_sha1_v4[:-1] + "e" if file_hash_sha1_v4.endswith("f") else file_hash_sha1_v4[:-1] + "f"
				wrong_sha256_hash = file_hash_sha256_v4[:-1] + "e" if file_hash_sha256_v4.endswith("f") else file_hash_sha256_v4[:-1] + "f"

				target_v4['hashes'] = Hashes(hashes={'md5': Binaryx(bytes.fromhex(wrong_md5_hash)), 'sha1': Binaryx(bytes.fromhex(wrong_sha1_hash)), 'sha256': Binaryx(bytes.fromhex(wrong_sha256_hash))})
				json_cmd_v4 = JSONEncoder.todict(Command(action, File(**target_v4), args, actuator=pf))
				...
				if target_v6:
					...
					wrong_md5_hash = file_hash_md5_v6[:-1] + "e" if file_hash_md5_v6.endswith("f") else file_hash_md5_v6[:-1] + "f"
					wrong_sha1_hash = file_hash_sha1_v6[:-1] + "e" if file_hash_sha1_v6.endswith("f") else file_hash_sha1_v6[:-1] + "f"
					wrong_sha256_hash = file_hash_sha256_v6[:-1] + "e" if file_hash_sha256_v6.endswith("f") else file_hash_sha256_v6[:-1] + "f"

					target_v6['hashes'] = Hashes(hashes={'md5': Binaryx(bytes.fromhex(wrong_md5_hash)), 'sha1': Binaryx(bytes.fromhex(wrong_sha1_hash)), 'sha256': Binaryx(bytes.fromhex(wrong_sha256_hash))})
					json_cmd_v6 = JSONEncoder.todict(Command(action, File(**target_v6), args, actuator=pf))
...
def test_sending_invalid_deny(cmd, create_producer, caplog):
	try:
        ...
        # Check if the status is BADREQUEST
		if resp.content.get('status') == StatusCode.BADREQUEST or \
			resp.content.get('status') == StatusCode.NOTIMPLEMENTED:
            # The test succeeds if BADREQUEST status is returned
			return
...
# Filter the log to get what I need
	...
	if c.actuator.getObj()['asset_id'] == 'iptables':
		...
		dst_addr = '10.17.2.26'
	arg = slpf.Args({'response_requested': ResponseType.complete, 'insert_rule': int(c.target.getObj()), 'direction': Direction.ingress})
	pf = slpf.Specifiers({'asset_id': c.actuator.getObj()['asset_id']})
...
def set_rules(create_producer, caplog, asset_id, direction, traffic_src_addr, traffic_dst_addr):
	...
	if asset_id == 'iptables':
		...
		if asset_id == 'kubernetes':
			delta_time += 10
			target = IPv4Connection(dst_addr='10.17.2.26') if direction == Direction.ingress else IPv4Connection(src_addr='10.17.2.26')
			arg = slpf.Args({
				'response_requested': ResponseType.complete,
				'direction': direction,
				'start_time': DateTime((now + delta_time) * 1000)
	...
	protocols = [ L4Protocol.icmp, L4Protocol.tcp, L4Protocol.udp ] if asset_id != 'kubernetes' else [ L4Protocol.tcp, L4Protocol.udp ]
...
def test_latency(cmd, create_producer, caplog):
	latency_measurement(
		...
		caplog,
		cmd=cmd,
		abs_path="/home/kali/Scrivania/openstack/all_del_producer_time.txt"
	)
...
#	time.sleep(2)	# openstack, kubernetes
	...
	with open(abs_path, "a") as f:
		f.write(f"{allow_response_timestamp - allow_command_timestamp} {delete_response_timestamp - delete_command_timestamp}\n")
