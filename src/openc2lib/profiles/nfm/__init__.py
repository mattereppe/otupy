""" Netflow Monitor

	This module collects all public definition that are exported as part of the NFM profile.
	All naming follows as much as possible the terminology in the NFM Specification, by
	also applying generic openc2lib conventions.

	This definition also registers all extensions defined in the NFM profile (`Args`, `Target`, `Profile`, `Results`).

"""

from openc2lib.profiles.nfm.profile import Profile
from openc2lib.profiles.nfm.actuator import *

from openc2lib import TargetEnum
from openc2lib.profiles.nfm.data import *
from openc2lib.profiles.nfm.targets import FlowMonitor
from openc2lib.profiles.nfm.targets import MonitorID


# According to the standard, extended targets must be prefixed with the nsid
from openc2lib.profiles.nfm.args import Args
from openc2lib.profiles.nfm.results import Results
from openc2lib.profiles.nfm.validation import AllowedCommandTarget, AllowedCommandArguments, validate_command, validate_args
