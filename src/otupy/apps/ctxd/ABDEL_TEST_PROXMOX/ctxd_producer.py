...
def main(openstack_parameters, collection):
    ...
    if not arg['name_only']: #explore actuators only if it is false
        ...
        dot.node('proxmox', 'Proxmox')
        recursive_process_links(resp_openstack.content['results']['links'], cmd, pf, p, dot, 'proxmox')
...
if __name__ == '__main__':
    ...
    for element in configuration_parameters['clusters']:
        if (element["type"] == "proxmox"):      
            main(element, collection) #start the discovery at the proxmox service
