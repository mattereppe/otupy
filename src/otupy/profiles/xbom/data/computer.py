...
class Computer(Record):
	...
	def as_cyclonedx(self) -> Component:
		"""Convert Computer to CycloneDX component format.
		
		Returns:
			Component: CycloneDX Component with type PLATFORM and nested components.
		"""
		properties = [
			Property(name="otupy:type", value="computer")
		]
		if self.id is not None:
			properties.append(Property(name="otupy:computer:id", value=self.id))
		if self.hostname is not None:
			properties.append(Property(name="otupy:computer:hostname", value=str(self.hostname)))
		
		# Add nested components (OS and applications)
		nested_components = []
		if self.os is not None:
			nested_components.append(self.os.as_cyclonedx())
