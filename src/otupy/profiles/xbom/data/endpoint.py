...
class Endpoint(Record):
	...
	""" Endpoint
	  ...
	  different elements that are necessary to identify the communication facets of very different network
		...
		Note: since most of these fields are descriptive and intended to be used by external software, no
	...
	def validate_fields(self):
		...
		if self.transport is not None and not isinstance(self.transport, str):
			raise TypeError(f"Expected 'transport' to be of type str, but got {type(self.transport)}")
		if self.transfer is not None and not isinstance(self.transfer, str):
			raise TypeError(f"Expected 'transfer' to be of type str, but got {type(self.transfer)}")
	...
	def as_cyclonedx(self, prefix: str = "otupy:endpoint") -> list:
