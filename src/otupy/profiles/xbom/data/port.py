	...
	def as_cyclonedx(self) -> List[Property]:
		...
		if self.addresses is not None:
			for addr in self.addresses:
				properties.append(Property(name=f"otupy:port:{port_id}:address", value=str(addr)))
		if self.gateway is not None:
			properties.append(Property(name=f"otupy:port:{port_id}:gateway", value=str(self.gateway)))
