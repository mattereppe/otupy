...
from otupy import ArrayOf, File, Nsid, Version, Command, Response, StatusCode, StatusCodeDescription, Features, \
    ResponseType, Feature, IPv4Net, IPv4Connection, IPv6Net, IPv6Connection, DateTime, Duration, Binaryx, L4Protocol, \
    Port
...
class SLPFActuator:
    ...
    def __init__(self, hostname=None, named_group=None, asset_id=None, asset_tuple=None, db_directory_path=None,
        ...
        if os.environ.get("WERKZEUG_RUN_MAIN") == "true":
            ...
            try:
                ...
                self.update_directory_path = update_directory_path if update_directory_path else os.path.dirname(
                ...
                self.db_directory_path = db_directory_path if db_directory_path else os.path.dirname(
                    os.path.abspath(__file__))
                ...
                self.db_commands_table_name = db_commands_table_name if db_commands_table_name else MY_IDS[
                                                                                                        'asset_id'] + "_commands"
                self.db_jobs_table_name = db_jobs_table_name if db_jobs_table_name else MY_IDS['asset_id'] + "_jobs"
                self.db = SQLDatabase(os.path.join(self.db_directory_path, self.db_name), self.db_commands_table_name,
                ...
                self.scheduler.add_listener(lambda event: self.scheduler_listener(event),
    ...
    def query(self, cmd):
        ...
        if (cmd.target.getObj().__class__ == Features):
            ...
            return Response(status=StatusCode.BADREQUEST,
                            status_text="Querying " + cmd.target.getName() + " not supported")
    ...
    def allow_deny_handler(self, cmd):
        ...
        try:
            ...
            #   Validating target
            if type(target) != IPv4Net and type(target) != IPv6Net and type(target) != IPv4Connection and type(
            ...
            if type(target) == IPv4Connection or type(target) == IPv6Connection:
                if target.protocol and target.protocol != L4Protocol.tcp and target.protocol != L4Protocol.udp and target.protocol != L4Protocol.sctp:
                    if target.src_port or target.dst_port:
                        raise ValueError(StatusCode.BADREQUEST,
                                         "Source/Destination port not supported with provided protocol")
            ...
            if 'insert_rule' in args:
                if not 'response_requested' in args or args['response_requested'] != ResponseType.complete:
                    raise ValueError(StatusCode.BADREQUEST,
                                     "Response requested must be complete with insert rule argument")
            ...
            if 'stop_time' in args:
                ...
                self.scheduler.add_job(self.delete_handler,
                                       ...
                                       kwargs={'command_to_delete': command, 'rule_number': rule_number,
                                               'custom_data': custom_data, 'scheduler_data': scheduler_data},
    ...
    def delete(self, cmd):
        ...
        try:
            ...
            self.scheduler.add_job(self.delete_handler,
                                   'date',
                                   next_run_time=start_time,
                                   kwargs={'command_to_delete': command_to_delete, 'rule_number': rule_number,
                                           'custom_data': custom_data, 'scheduler_data': scheduler_data},
    ...
    def update(self, cmd):
        ...
        try:
            ...
            abs_path = os.path.join(self.update_directory_path, target['name']) if not 'path' in target else target[
                'path']
    ...
    def db_handler(self, action, target, args, custom_data, scheduler_data):
        ...
        try:
            ...
            rule_number = self.db.insert_command(
                ...
                stop_time=datetime.fromtimestamp(args['stop_time']).isoformat(sep=' ',
                                                                              timespec='milliseconds') if 'stop_time' in args else None,
