...
from otupy import Actions, StatusCode, IPv4Net, IPv4Connection, IPv6Net, IPv6Connection, Response, \
    StatusCodeDescription, Feature, ArrayOf, Version, Nsid, ActionTargets, TargetEnum
...
class SLPFActuator_openstack(SLPFActuator):
    ...
    def connect_to_openstack(self):
        ...
        try:
            ...
            if (
                    self.environment_variables_file is not None):  # if it is none, it will use the enviroment variables already present in the system
    ...
    def openstack_allow_handler(self, target, direction, custom_data):
        ...
        try:
            security_group_id = custom_data['ingress_id'] if direction == Direction.ingress else custom_data[
                'egress_id']
    ...
    def openstack_from_openc2(self, target, direction, security_group_id):
        ...
        try:
            security_group_rule = SecurityGroupRule(
                ...
                protocol=target.protocol.name.lower() if (type(target) == IPv4Connection or type(
